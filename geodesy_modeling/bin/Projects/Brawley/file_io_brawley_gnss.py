...
import numpy as np
from gnss_timeseries_viewers.gps_tools import gps_ts_functions, offsets, gps_seasonal_removals, remove_postseismic, \
    load_gnss


def read_station_ts_NBGF(gps_bbox, gps_reference, remove_coseismic=0, network='pbo', _blacklist=()):
    """
    ...
    database = load_gnss.create_station_repo(gps_data_config_file, proc_center=network, refframe='NA')
    stations, _ = database.search_stations_by_box(gps_bbox)
    [dataobj_list, offsetobj_list, eqobj_list] = database.load_stations(stations)

    # Next 5 lines: Importing BRAW from UNR
    unr_database = load_gnss.create_station_repo(gps_data_config_file, proc_center='unr', refframe='NA')
    [myData, offset_obj, eq_obj] = unr_database.load_stations(["BRAW"])
    ...
    for i in range(len(dataobj_list)):
        ...
        model_obj = remove_postseismic.get_station_hines(newobj.name, gps_data_config_file)
        newobj = remove_postseismic.remove_by_model(newobj, model_obj, starttime1, endtime1, starttime2, endtime2)
