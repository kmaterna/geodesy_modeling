...
def read_grd_inputs(params):
    ...
    """
    myobj = inputs.inputs_from_synthetic_enu_grids(params['east_grdfile'], params['north_grdfile'],
                                                   params['up_grdfile'], params['flight_direction'],
                                                   params['incidence_angle'], look_direction=params['look_direction'])
    return myobj
...
def plot_synthetic_grid_los(params, insarobj, disp_points=None, disp_points_color=None):
    ...
    if params['plot_unwrapped']:  # the outputs are in los deformation (mm)
        ...
        outputs.write_InSAR2D(myobj_ref, os.path.join(params['outdir'], params['label'] + "unw_los.grd"))
        outputs.map_wrapped_insar(os.path.join(params['outdir'], params['label'] + "unw_los.grd"),
                                  os.path.join(params['outdir'], params['label']+params['unwrapped_plot_name']),
                                  text_annot=params['plot_unwrapped_annot'], flight_heading=params['flight_direction'],
                                  look_dir=params['look_direction'], disp_points=disp_points,
                                  region=params['plot_range'], refloc=params['refidx'],
                                  disp_points_color=disp_points_color)

    if params['plot_wrapped']:  # the outputs are in phase (radians)
        myobj_wrapped = insarobj.rewrap_InSAR(params['wavelength_mm'])
        outputs.write_InSAR2D(myobj_wrapped, os.path.join(params['outdir'], params['label'] + "phase.grd"))
        outputs.map_wrapped_insar(os.path.join(params['outdir'], params['label'] + "phase.grd"),
                                  os.path.join(params['outdir'], params['label']+params["wrapped_plot_name"]),
                                  text_annot=params['plot_wrapped_annot'], look_dir=params['look_direction'],
                                  flight_heading=params['flight_direction'], disp_points=disp_points,
                                  region=params['plot_range'], wrapped=True)
    return


def do_synthetic_grid_LOS(params, disp_points=None, disp_points_color=None):
