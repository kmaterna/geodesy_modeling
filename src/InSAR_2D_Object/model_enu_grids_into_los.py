...
"""
Take 3 grd files of modeled east/north/up deformation and project them into the LOS of an assumed satellite flight.
...
def parse_config(params_provided):
    defaults = {  # REQUIRED PARAMETERS
              ...
              'refidx': None,   # either [float, float] for lon, lat, or [int, int] for row, col, or None
              'plot_range': None,
    ...
    # declaring priority order.  Combine dictionary with priority using ** operator
...
def plot_synthetic_grid_los(params, insarobj, disp_points=None, disp_points_color=None):
    ...
    if params['plot_unwrapped']:
        if params['refidx'] is None:
            params['refidx'] = [0, 0];   # default to the corner pixel if strictly necessary.
