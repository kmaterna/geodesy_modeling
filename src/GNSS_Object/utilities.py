"""
Conversion between Station_Vels and Disp_Points
...
def disp_points_to_Station_Vels(list_of_disp_points):
    ...
    for dpo in list_of_disp_points:
        new_station_vel = gps_io_functions.Station_Vel(elon=dpo.lon, nlat=dpo.lat, e=dpo.dE_obs*1000,
                                                       n=dpo.dN_obs*1000, u=dpo.dU_obs*1000, se=dpo.Se_obs*1000,
                                                       sn=dpo.Sn_obs*1000, su=dpo.Su_obs*1000, name=dpo.name,
                                                       first_epoch=dpo.starttime, last_epoch=dpo.endtime,
                                                       refframe=dpo.refframe, meas_type=dpo.meas_type);
        # in units of mm/yr
...
def Station_Vels_to_disp_points(list_of_station_vels):
    ...
    for station_vel in list_of_station_vels:
        new_dpo = coulomb_collections.Displacement_points(lon=station_vel.elon, lat=station_vel.nlat,
                                                          ...
                                                          endtime=station_vel.last_epoch,
                                                          refframe=station_vel.refframe,
                                                          meas_type=station_vel.meas_type);   # in units of m
