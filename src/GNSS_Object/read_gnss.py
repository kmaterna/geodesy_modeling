...
def read_station_ts(gps_bbox, gps_reference, remove_coseismic=0, network='pbo'):
    ...
    [myData, offset_obj, eq_obj] = gps_tools.gps_input_pipeline.get_station_data("BRAW", "unr",
                                                                                 gnss_object.gps_data_config_file,
                                                                                 refframe="NA")
    ...
    for i in range(len(dataobj_list)):
        ...
        newobj = gps_tools.offsets.remove_offsets(one_object,
                                                  offsetobj_list[i]);  # will remove antenna offsets from everything

        if newobj.name == 'BRAW':
            newobj = gps_tools.gps_seasonal_removals.make_detrended_ts(newobj, seasonals_remove=1,
                                                                       seasonals_type="lssq",
