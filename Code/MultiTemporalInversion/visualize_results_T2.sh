...
# FAULT PLOTS
range="-115.73/-115.4267/32.9256/33.159"
...
awk '{print $1, $2, $3*1000}' $obs_insar_file | gmt psxy -Sc0.14 -Clos.cpt -R -J$proj -O -K >> $output
...
gmt psxy $ss_fault_file -R -J$proj -Wthinnest,gray -L -K -O >> $output # Putting the faults on there for kicks
awk '{print $1, $2, $3*1000}' $model_insar_file | gmt psxy -Sc0.14 -Clos.cpt -R -J$proj -O -K >> $output
...
gmt pscoast -J$proj -R -Gwhite -Slightgray -N1 -Wthin,black -B0.2wesn -X2.35i -Dh -K -O -P >> $output
gmt psxy $ss_fault_file -R -J$proj -Wthinnest,gray -L -K -O >> $output # Putting the faults on there for kicks
awk '{print $1, $2, $3-$4}' temp_lev.txt | gmt psxy -Sc0.14 -Clos.cpt -R -J$proj -O -K >> $output
...
# UAVSAR TRACK 26509
range="-115.73/-115.4267/32.9256/33.14"
files=`awk 'NR==3' "$1"`
obs_insar_file=`echo $files | awk '{print $1;}'`
model_insar_file=`echo $files | awk '{print $2;}'`
label=`echo $files | awk '{print $3, $4, $5;}'`
...
echo "-115.47 33.102 LOS" | gmt pstext -R -J$projection -F+f8p,Helvetica-bold -K -O -P >> $output
awk '{print $1, $2, $3*1000}' $obs_insar_file | gmt psxy -Sc0.09 -Clos.cpt -R -J$proj -O -K >> $output
gmt pscoast -J$proj -R -Gwhite -Slightgray -N1 -Wthin,black -B0.2wesn -X2.35i -Dh -K -O -P >> $output
# echo "-115.52 33.14 UAVSAR Model" | gmt pstext -R -J$projection -F+f12p,Helvetica-bold -K -O -P >> $output
echo "-115.47 33.13 -0.99 -0.09 0 0 0" | gmt psvelo -R -J -Se1/0.95/0 -A0.3+a30+ea+gblack+p-2p,black -K -O >> $output  # Flight vector
echo "-115.47 33.13 0.05 -0.49 0 0 0" | gmt psvelo -R -J -Se1/0.95/0 -A0.5+a30+ea+gblack+p-3p,black -K -O >> $output  # LOS
gmt psxy $ss_fault_file -R -J$proj -Wthinnest,gray -L -K -O >> $output # Putting the faults on there for kicks
awk '{print $1, $2, $3*1000}' $model_insar_file | gmt psxy -Sc0.09 -Clos.cpt -R -J$proj -O -K >> $output
...
echo "-115.47 33.13 0.05 -0.49 0 0 0" | gmt psvelo -R -J -Se1/0.95/0 -A0.5+a30+ea+gblack+p-3p,black -K -O >> $output  # LOS
gmt psxy $ss_fault_file -R -J$proj -Wthinnest,gray -L -K -O >> $output # Putting the faults on there for kicks
awk '{print $1, $2, $3-$4}' temp_insar.txt | gmt psxy -Sc0.09 -Clos.cpt -R -J$proj -O -K >> $output


# UAVSAR TRACK 08508
southrange="-115.73/-115.4267/32.91/33.11"
files=`awk 'NR==4' "$1"`
obs_insar_file=`echo $files | awk '{print $1;}'`
model_insar_file=`echo $files | awk '{print $2;}'`
label=`echo $files | awk '{print $3, $4, $5;}'`
gmt pscoast -J$proj -R$southrange -Gwhite -Slightgray -N1 -Wthin,black -B0.2wesn -X-4.7i -Y-1.9i -Dh -K -O -P >> $output
echo "-115.78 33.01 UAVSAR" | gmt pstext -R -J$projection -F+a90+f17p,Helvetica-bold -K -N -O -P >> $output
echo "-115.75 33.01 T08508" | gmt pstext -R -J$projection -F+a90+f17p,Helvetica-bold -K -N -O -P >> $output
...
echo "-115.68 32.95 LOS" | gmt pstext -R -J$projection -F+f8p,Helvetica-bold -K -O -P >> $output
gmt psxy $ss_fault_file -R -J$proj -Wthinnest,gray -L -K -O >> $output # Putting the faults on there for kicks
