...
echo "-115.55 33.12 Model Reverse Slip" | gmt pstext -R$range -J$projection -F+f12p,Helvetica-bold -K -O -P >> $output
...
echo "-115.55 33.12 Model Strike Slip (LL)" | gmt pstext -R$range -J$projection -F+f12p,Helvetica-bold -K -O -P >> $output
gmt psxy $field_file -R -J$proj -Wthin,indianred -O -K >> $output
...
gmt pscoast -J$proj -R$bigrange -Gwhite -Slightgray -N1 -Wthin,black -B0.2wEsN -Y0i -X2.4i -Dh -K -O -P >> $output
files=`awk 'NR==1' "$1"`
...
# S1 Ascending
range="-115.67/-115.43/32.95/33.11"
files=`awk 'NR==3' "$1"`
obs_insar_file=`echo $files | awk '{print $1;}'`
model_insar_file=`echo $files | awk '{print $2;}'`
label=`echo $files | awk '{print $3, $4, $5;}'`
gmt pscoast -J$proj -R$range -Gwhite -Slightgray -N1 -Wthin,black -B0.2wesn -X-4.7i -Y-1.9i -Dh -K -O -P >> $output
echo "-115.73 33.04 S1" | gmt pstext -R -J$projection -F+a90+f17p,Helvetica-bold -K -N -O -P >> $output
echo "-115.70 33.04 Ascending" | gmt pstext -R -J$projection -F+a90+f17p,Helvetica-bold -K -N -O -P >> $output
gmt psxy $ss_fault_file -R -J$proj -Wthinnest,gray -L -K -O >> $output # Putting the faults on there for kicks
echo "-115.635 32.98 -0.22 0.97 0 0 0" | gmt psvelo -R -J -Se1/0.95/0 -A0.3+a30+ea+gblack+p-2p,black -K -O >> $output  # Flight vector
echo "-115.635 32.98 0.48 0.11 0 0 0" | gmt psvelo -R -J -Se1/0.95/0 -A0.5+a30+ea+gblack+p-3p,black -K -O >> $output  # LOS
echo "-115.61 32.97 LOS" | gmt pstext -R -J$projection -F+f11p,Helvetica-bold -K -O -P >> $output
...
# S1 Descending
southrange="-115.68/-115.4267/32.93/33.11"
files=`awk 'NR==4' "$1"`
obs_insar_file=`echo $files | awk '{print $1;}'`
model_insar_file=`echo $files | awk '{print $2;}'`
label=`echo $files | awk '{print $3, $4, $5;}'`
gmt pscoast -J$proj -R$southrange -Gwhite -Slightgray -N1 -Wthin,black -B0.2wesn -X-4.7i -Y-2.0i -Dh -K -O -P >> $output
echo "-115.74 33.01 S1" | gmt pstext -R -J$projection -F+a90+f17p,Helvetica-bold -K -N -O -P >> $output
echo "-115.71 33.01 Descending" | gmt pstext -R -J$projection -F+a90+f17p,Helvetica-bold -K -N -O -P >> $output
echo "-115.442 33.095 -0.22 -0.97 0 0 0" | gmt psvelo -R -J -Se1/0.95/0 -A0.3+a30+ea+gblack+p-2p,black -K -O >> $output  # Flight vector
echo "-115.442 33.095 -0.47 0.11 0 0 0" | gmt psvelo -R -J -Se1/0.95/0 -A0.5+a30+ea+gblack+p-3p,black -K -O >> $output  # LOS
echo "-115.48 33.10 LOS" | gmt pstext -R -J$projection -F+f11p,Helvetica-bold -K -O -P >> $output
gmt psxy $ss_fault_file -R -J$proj -Wthinnest,gray -L -K -O >> $output # Putting the faults on there for kicks
awk '{print $1, $2, $3*1000}' $obs_insar_file | gmt psxy -Sc0.11 -Clos.cpt -R -J$proj -O -K >> $output
gmt psxy $field_file -R -J$proj -Wthin,indianred -O -K >> $output
gmt pscoast -J$proj -R -Gwhite -Slightgray -N1 -Wthin,black -B0.2wesn -X2.35i -Dh -K -O -P >> $output
