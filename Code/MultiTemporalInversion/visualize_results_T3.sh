...
echo "-115.53 33.14 Strike Slip (LL)" | gmt pstext -R$range -J$projection -F+f12p,Helvetica-bold -K -O -P >> $output
gmt pscoast -J$proj -R$range -Gwhite -Slightgray -N1 -Wthin,black -B0.2wesn -X2.35i -Y0i -Dh -K -O -P >> $output
gmt psxy $thrust_file -R$range -J$proj -W0.01p,gray -Cslip.cpt -L -K -O >> $output
echo "-115.53 33.14 Reverse Slip" | gmt pstext -R$range -J$projection -F+f12p,Helvetica-bold -K -O -P >> $output
