...
# FAULT PLOTS
smallrange="-115.63/-115.46/32.96/33.10"
gmt pscoast -J$proj -R$smallrange -Gwhite -Slightgray -N1 -Wthin,black -B0.2wesn -X0.7i -Y9i -Dh -K -P > $output
gmt psxy $ss_fault_file -R -J$proj -W0.01p,gray -Cslip.cpt -L -K -O >> $output
echo "-115.55 33.09 Model Strike Slip (LL)" | gmt pstext -R -J$projection -F+f12p,Helvetica-bold -K -O -P >> $output
...
echo "-115.55 33.09 Model Reverse Slip" | gmt pstext -R -J$projection -F+f12p,Helvetica-bold -K -O -P >> $output
gmt psscale -Cslip.cpt -Dx-0.5c/-0.5c+w5c/0.25c+jTC+h -Bxaf -By+l"m" -K -O >> $output



# GPS PLOTS
bigrange="-115.8/-115.35/32.8/33.17"
...
echo "-115.47 33.15 GNSS" | gmt pstext -R -J$projection -F+f12p,Helvetica-bold -K -O -P >> $output
...
`echo -115.72 + 0.11 | bc` `echo 32.85 + 0.00 | bc` .02 0 0 0 0 20mm model
EOF
gmt psvelo -A+e+gblack+pthickest -Wblack -Se$horiz_scale/0.68/10 -R -J$proj -O -K <<EOF>> $output
`echo -115.72 + 0.11 | bc` `echo 32.85 + 0.02 | bc` .02 0 0.001 0.001 0 20mm obs
EOF
...
label=`echo $files | awk '{print $3, $4, $5;}'`
gmt pscoast -J$proj -R$smallrange -Gwhite -Slightgray -N1 -Wthin,black -B0.2wesn -X-4.7i -Y-3.3i -Dh -K -O -P >> $output
...
echo "-115.54 33.10 Observations" | gmt pstext -R -J$projection -F+f18p,Helvetica-bold -N -K -O -P >> $output
...
echo "-115.54 33.10 Models" | gmt pstext -R -J$projection -F+f18p,Helvetica-bold -N -K -O -P >> $output
...
echo "-115.54 33.10 Residuals" | gmt pstext -R -J$projection -F+f18p,Helvetica-bold -N -K -O -P >> $output



# UAVSAR TRACK 26509
range="-115.72/-115.4467/32.94/33.14"
files=`awk 'NR==3' "$1"`
obs_insar_file=`echo $files | awk '{print $1;}'`
model_insar_file=`echo $files | awk '{print $2;}'`
label=`echo $files | awk '{print $3, $4, $5;}'`
gmt pscoast -J$proj -R$range -Gwhite -Slightgray -N1 -Wthin,black -B0.2wesn -X-4.7i -Y-2.1i -Dh -K -O -P >> $output
# echo "-115.53 33.14 UAVSAR Data" | gmt pstext -R -J$projection -F+f12p,Helvetica-bold -K -O -P >> $output
echo "-115.47 33.13 -0.99 -0.09 0 0 0" | gmt psvelo -R -J -Se1/0.95/0 -A0.3+a30+ea+gblack+p-2p,black -K -O >> $output  # Flight vector
echo "-115.47 33.13 0.05 -0.49 0 0 0" | gmt psvelo -R -J -Se1/0.95/0 -A0.5+a30+ea+gblack+p-3p,black -K -O >> $output  # LOS
echo "-115.47 33.102 LOS" | gmt pstext -R -J$projection -F+f11p,Helvetica-bold -K -O -P >> $output
echo "-115.77 33.04 UAVSAR" | gmt pstext -R -J$projection -F+a90+f17p,Helvetica-bold -K -N -O -P >> $output
echo "-115.74 33.04 T26509" | gmt pstext -R -J$projection -F+a90+f17p,Helvetica-bold -K -N -O -P >> $output
gmt psxy $ss_fault_file -R -J$proj -Wthinnest,gray -L -K -O >> $output # Putting the faults on there for kicks
...
# UAVSAR TRACK 08508
southrange="-115.72/-115.4467/32.88/33.10"
files=`awk 'NR==4' "$1"`
obs_insar_file=`echo $files | awk '{print $1;}'`
model_insar_file=`echo $files | awk '{print $2;}'`
label=`echo $files | awk '{print $3, $4, $5;}'`
...
gmt psxy $ss_fault_file -R -J$proj -Wthinnest,gray -L -K -O >> $output # Putting the faults on there for kicks
awk '{print $1, $2, $3*1000}' $obs_insar_file | gmt psxy -Sc0.12 -Clos.cpt -R -J$proj -O -K >> $output
echo "-115.68 32.89 0.99 0.09 0 0 0" | gmt psvelo -R -J -Se1/0.95/0 -A0.3+a30+ea+gblack+p-2p,black -K -O >> $output  # Flight vector
echo "-115.68 32.89 -0.05 0.49 0 0 0" | gmt psvelo -R -J -Se1/0.95/0 -A0.5+a30+ea+gblack+p-3p,black -K -O >> $output  # LOS
echo "-115.68 32.92 LOS" | gmt pstext -R -J$projection -F+f11p,Helvetica-bold -K -O -P >> $output
echo "-115.77 33.01 UAVSAR" | gmt pstext -R -J$projection -F+a90+f17p,Helvetica-bold -K -N -O -P >> $output
echo "-115.74 33.01 T08508" | gmt pstext -R -J$projection -F+a90+f17p,Helvetica-bold -K -N -O -P >> $output
gmt pscoast -J$proj -R -Gwhite -Slightgray -N1 -Wthin,black -B0.2wesn -X2.35i -Dh -K -O -P >> $output
