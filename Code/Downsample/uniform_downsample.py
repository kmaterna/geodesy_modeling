	...
	for i in range(len(y_array)):
		for j in range(len(x_array)):
			if averaging_window==0:  # If we just want to find THE nearest pixel
				...
				new_obs_array[i][j] = get_average_within_box(InSAR_obj.lon, InSAR_obj.lat, x_array[j], y_array[i], averaging_window, InSAR_obj.LOS);
				new_obs_unc[i][j] = get_average_within_box(InSAR_obj.lon, InSAR_obj.lat, x_array[j], y_array[i], averaging_window, InSAR_obj.LOS_unc);
				new_e[i][j] = get_average_within_box(InSAR_obj.lon, InSAR_obj.lat, x_array[j], y_array[i], averaging_window, InSAR_obj.lkv_E);
				new_n[i][j] = get_average_within_box(InSAR_obj.lon, InSAR_obj.lat, x_array[j], y_array[i], averaging_window, InSAR_obj.lkv_N);
				new_u[i][j] = get_average_within_box(InSAR_obj.lon, InSAR_obj.lat, x_array[j], y_array[i], averaging_window, InSAR_obj.lkv_U);
