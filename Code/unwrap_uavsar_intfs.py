...
def read_jpl_ground_range_data(data_file_slant, corr_file_slant, ann_file):
	real, imag = jpl_uav_read_write.read_igram_data(data_file_slant, ann_file, igram_type='ground', return_type='real_imag');
...
def main(ann_file, data_file, corr_file, after_filtering, after_filtering_corr, cut_rowcol, cor_cutoff, looks_x, looks_y, wavelength):
	...
	filter_with_looks_py(after_filtering, after_filtering_corr, looks_x, looks_y);  # Filter the igrams using ISCE's toolkit

	# # MASK, INTERPOLATE, AND UNWRAP STEP
	masked_phase, mask = multiply_igram_by_coherence_mask(after_filtering, after_filtering_corr, cor_cutoff); # multiply by coherence mask
	...
	re_masked = read_and_reapply_mask(mask);  # re-apply the mask and write
	...
	isce_geocode_tools.write_unwrapped_ground_range_displacements("unwrap_masked.grd", "uavsar.unw.geo", x_axis, y_axis, wavelength);
