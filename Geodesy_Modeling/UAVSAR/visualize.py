...
def visualize_timedep_timeseries(config_filename, gps_filename, outdir):
    ...
    myUAVSAR_TS = UAVSAR.uavsar_readwrite.inputs_TS_grd(uav_los, uav_lon, uav_lat);
    [gps_lons, gps_lats, gps_names] = np.loadtxt(gps_filename, unpack=True, dtype={"formats":(np.float, np.float, 'U4'),
